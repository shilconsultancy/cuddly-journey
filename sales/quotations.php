...
if (!check_permission('Sales', 'view')) {
    die('<div class="glass-card p-8 text-center">You do not have permission to view quotations.</div>');
}

$page_title = "Quotations - BizManager";
...
$sql = "
    SELECT q.id, q.quote_number, q.quote_date, q.status, q.total_amount, cust.customer_name
    FROM scs_quotations q
    JOIN scs_customers cust ON q.customer_id = cust.id
";
$where_clauses = [];
$params = [];
$types = '';
if (!empty($search_term)) {
    $where_clauses[] = "(q.quote_number LIKE ? OR cust.customer_name LIKE ?)";
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
    <div>
        ...
        <p class="text-gray-600 mt-1">Create and manage all customer quotations.</p>
    </div>
    <div class="mt-4 md:mt-0 flex space-x-2">
        ...
        <a href="quotation-form.php" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white font-semibold rounded-lg shadow-sm hover:bg-indigo-700 transition-colors">
...
<div class="glass-card p-4 mb-6">
    <form action="quotations.php" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
        <div class="md:col-span-2">
            <label for="search" class="block text-sm font-medium text-gray-700">Search</label>
            <input type="text" name="search" id="search" value="<?php echo htmlspecialchars($search_term); ?>" placeholder="Quote # or Customer Name" class="form-input mt-1 block w-full">
        </div>
        <div>
        ...
        <div class="flex space-x-2">
            ...
            <a href="quotations.php" class="w-full inline-flex justify-center py-2 px-4 rounded-md text-gray-700 bg-gray-200 hover:bg-gray-300">Clear</a>
        </div>
    </form>
</div>
...
<div class="glass-card p-6">
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-gray-700">
            ...
            <tbody>
                <?php if ($quotations_result->num_rows > 0): ?>
                    <?php while($row = $quotations_result->fetch_assoc()): ?>
                    <tr class="bg-white/50 border-b border-gray-200/50">
                        <td class="px-6 py-4 font-bold text-indigo-600"><?php echo htmlspecialchars($row['quote_number']); ?></td>
                        <td class="px-6 py-4 font-medium"><?php echo htmlspecialchars($row['customer_name']); ?></td>
                        <td class="px-6 py-4"><?php echo date($app_config['date_format'], strtotime($row['quote_date'])); ?></td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="quotation-details.php?id=<?php echo $row['id']; ?>" class="font-medium text-green-600 hover:underline">View</a>
                            <?php if (check_permission('Sales', 'edit')): ?>
                                <a href="quotation-form.php?id=<?php echo $row['id']; ?>" class="font-medium text-indigo-600 hover:underline">Edit</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="6" class="px-6 py-4 text-center text-gray-500">No quotations found matching your criteria.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
